...
| `gender` | ✅ | enum | `male` / `female` / `other`（床位分配依赖） |
| `policeConsent` | ✅ | boolean | 警方登记同意 |
| `dob` | ❌ | string | ISO 日期 |
| `arrivalTime` | ❌ | enum | `morning` / `afternoon` / `evening` / `late` |
| `referral` | ❌ | string | 自由文本（"如何得知本店"） |
| `source` | ❌ | enum | `walk-in` / `phone` / `email` / `referral` / `other`（默认 `walk-in`） |
| `notes` | ❌ | string | 自由文本 |
...
┌─ ID & SOURCE ─────────────────┐
│ ID Type *  ID Number *       │
│ Source      (walk-in/phone/email/referral/other, 默认 walk-in) │
│ Referral                     │
│ Notes                        │
└───────────────────────────────┘
...
**Gender 必填原因**：`scoreBeds` 算法按性别匹配女宿/混宿，未填则床位推荐退化。
...
| `checkin.source` | Booking source | 预订来源 |
| `checkin.source.walkIn` | Walk-in | 现场到店 |
| `checkin.source.phone` | Phone | 电话预订 |
| `checkin.source.email` | Email | 邮件预订 |
| `checkin.source.referral` | Referral | 推荐介绍 |
| `checkin.source.other` | Other | 其它 |
...
| `src/locales/en.json` | 修改 | +24 keys |
| `src/locales/zh.json` | 修改 | +24 keys |
